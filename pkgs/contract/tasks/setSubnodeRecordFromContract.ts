...
task("setSubnodeRecordFromContract", "setSubnodeRecord on ENS")
  .addParam("label", "label (string)")
  .setAction(async (taskArgs: any, hre: HardhatRuntimeEnvironment) => {
    ...
    // コントラクトのアドレスを取得する
    const {
      contracts: {SampleCreateSubDomain},
    } = loadDeployedContractAddresses(hre.network.name);

    // コントラクトインスタンスを生成する。
    const sampleCreateSubDomain = await hre.ethers.getContractAt(
    ...
    try {
      // sub domainを登録する。
      const tx = await sampleCreateSubDomain.createSubDomain(taskArgs.label);
...
const ABI = [
  ...
  },
  {
    ...
    outputs: [
      {internalType: "contract INameWrapperUpgrade", name: "", type: "address"},
    ],
    stateMutability: "view",
    type: "function",
  },
