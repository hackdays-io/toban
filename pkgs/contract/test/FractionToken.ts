...
describe("FractionToken", () => {
  ...
  it("should mint, transfer and burn tokens", async () => {
    ...
    const tokenId = await FractionToken.read.getTokenId([hatId, address2.account?.address!]);
  ...
  it("should fail to mint a token", async () => {
    // roleのない人ににtokenはmintできない
    await FractionToken.write.mint([hatId, address3.account?.address!])
      .catch(error => {
        expect(error.details).to.include("not authorized");
      });

    // tokenは二度mintできない
    await FractionToken.write.mint([hatId, address1.account?.address!])
      .catch(error => {
        expect(error.details).to.include("already received");
      })
  });
  ...
  it("should fail to burn a token", async () => {
    ...
    await FractionToken.write.burn(
      ...
      {
        account: address2.account?.address!
      }
    ).catch(error => {
      expect(error.details).to.include("not authorized");
    });
