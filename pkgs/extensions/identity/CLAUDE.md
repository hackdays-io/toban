...
Routes served by `worker.ts`: `POST /api/connect`, `GET /api/lookup`, `GET|POST /api/lookup/by-wallet`, `GET`/`POST /api/platform-link`, `POST /api/platform-link/notify-channel`, `POST /api/install-state/claim-jti`, `GET /health`.
