...
- **Privy** (`@privy-io/react-auth`) for auth + embedded/smart wallets. `useActiveWallet` (`hooks/useWallet.ts`) is the canonical hook. While the smart wallet is provisioning, `AccountMenu` renders an inline spinner in its top-left / header slot rather than blocking the whole AppShell — drive any new "wallet ready" gates off `useActiveWallet`'s `isConnectingEmbeddedWallet`/`isSmartWallet` directly.
